English | [简体中文](usage_example.zh-CN.md)

### MCU Boot User Interface

Compare `read` and `write` commands，you can see that the `address` can be omitted in the `write` subcommand, so just type `write <filename>`, don't worry, `mboot` will recognize it correctly. If you are not sure, you can use `-h` to see the help of subcommands at any time.
...
The following is a simple demonstration of the following read and write operations on the memory, please note the difference between them and `MCU Boot User Interface`, such as the output log level is higher than `MCU Boot User Interface` by default.
